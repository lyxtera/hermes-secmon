...
- **Tool-layer output redaction masks secret-looking test literals.** When reading/writing files that contain secret-shaped values (`sk-...`, `AKIA...`, `apiKey: <value>`), Hermes' output layer may display them as `«redacted:…»` or `***` even though the on-disk file is correct. In the 2026-08-09 session the new `tests/test_secret_scan_false_positives.py` read back with `«redacted:sk-…»` markers — looked like garbage/placeholder tests, but `sed -n ... | cat -A` and the passing pytest run proved the real content was fine. Never judge a test file by the redacted read-back; verify via pytest.
